    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside className="relative flex h-full w-full max-w-md flex-col border-l border-ink/10 bg-paper shadow-xl">
        <header className="flex items-center justify-between border-b border-ink/10 px-4 py-3">
          ...
          <button onClick={onClose} className="rounded-lg p-1.5 text-ink/50 hover:bg-ink/5" aria-label="关闭">
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-4 py-4">
          ...
          <section>
            ...
            <div className="rounded-2xl border border-ink/10 bg-white/70 p-4 text-sm text-ink/80">
              ...
              {profile?.skillGaps && profile.skillGaps.length > 0 && (
                <div className="mt-2">
                  {profile.overallScore != null && <p className="mb-1">综合能力：{profile.overallScore} 分</p>}
          ...
          <section>
            ...
            <div className="mt-2">
              {!user && <p className="text-xs text-ink/40">登录后可使用云端语义搜索；未登录时不可用</p>}
              ...
              {results.length > 0 && (
                <ul className="space-y-2">
                  {results.map((r) => (
                    <li key={r.id} className="rounded-xl border border-ink/10 bg-white/70 p-3 text-sm text-ink/80">
                      {r.content}
